...
当用户要求对项目进行 review / 审核时，需要重点审查项目架构、方案设计、代码设计的合理性，并给出明确反馈意见。

review 结论保存到项目根目录下的 `doc\review\` 路径中，文件名需包含日期和主题，例如：`YYYY-MM-DD_主题.md`。

保存的 review 文档中必须包含以下信息：
...
review 结果应优先指出实际风险、设计偏差、行为回归风险、测试缺口和文档/实现不一致问题；如果没有明显问题，也需要明确说明当前未发现关键问题，并注明剩余风险。
