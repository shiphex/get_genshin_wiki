...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Reparse and validate stored wiki pages by entity category.")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Maximum number of titles to process per entity.",
    )
    return parser
...
def main() -> int:
    ...
    report = run_batch(
        ...
        report_key="todo-parse-only-report",
    )
    print_report_summary(report)
    return 0 if report["validation"]["failed"] == 0 else 1
